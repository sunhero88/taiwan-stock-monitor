# main.py
# =========================================================
# Sunhero｜股市智能超盤中控台（Predator V16.3.43 架構校準版 + UCC V19.1）
# =========================================================
...
# 1. 系統常數與初始化
# =========================
st.set_page_config(page_title="Sunhero｜Predator V16.3.43 + UCC V19.1", layout="wide", initial_sidebar_state="expanded")
APP_TITLE = "Sunhero｜股市智能超盤中控台 (Predator V16.3.43 架構校準版 + UCC V19.1)"
...
def main():
    ...
    with st.sidebar:
        ...
        st.divider()
        st.header("🧭 UCC 裁決模式")
        run_mode = st.radio("RUN", ["L1", "L2", "L3"], index=0, horizontal=True, help="L1=只審核資料；L2=交易裁決；L3=回撤壓測")
    ...
    with st.spinner("雷達掃描與終極架構校準中..."):
        ...
        # -------------------------
        # Institutional date logic (T-1 before ~15:00)
        # -------------------------
        ...
        for i, sym in enumerate(list(STOCK_NAME_MAP.keys())[:topn], 1):
            ...
            inst_fresh = bool(has_inst and (not is_stale))
    ...
    if not df_disp.empty:
        ...
        disp_cols = {
            'Symbol': '代號', 'Name': '名稱', 'rank': '排名', 'tier_level': '強弱級別(1/2)',
    ...
    # =========================
    # 產生最終 JSON (UCC / Arbiter Input) + 補齊欄位
    # =========================

    # (A) market_status_reason：用「機器可讀」列出降級原因
    ...
    if is_stale:
        market_status_reason.append("INSTITUTIONAL_T_MINUS_1")
    if amt.confidence_level == "LOW":
        market_status_reason.append("AMOUNT_CONF_LOW")

    # (B) confidence 多維（不影響既有 meta.confidence_level；只是補充）
    ...
    inst_conf = "LOW" if is_stale else ("MEDIUM" if (not token) else "HIGH")

    # (C) max_equity_allowed_pct：把 final_limit 正式寫入 payload（L2 合法唯一來源）
    m2 = dict(m)
    m2["max_equity_allowed_pct"] = float(final_limit)
    ...
    payload = {
        "meta": {
            ...
            # 給 L1 的時序標註（避免「用前一日」卻缺 effective_trade_date 的違規）
            "is_using_previous_day": bool(is_stale),
            "effective_trade_date": inst_date if is_stale else trade_date,

            # 多維信心補充（供你未來擴寫；UCC 不會強制依賴它）
            "confidence": {
                "price": price_conf,
                "volume": volume_conf,
    ...
    # =========================
    st.markdown("---")
    st.subheader("🧠 UCC V19.1 裁決輸出（單一層級）")
    ...
    st.markdown("---")
    st.subheader("🤖 AI JSON（UCC/決策引擎終極安全輸入源）")
