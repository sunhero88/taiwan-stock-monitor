...
class StockNotifier:
    ...
    def send_stock_report(self, market_name, img_data, report_df, text_reports):
        ...
        # 💡 核心對接點：讀取 main.py 寫入的 FINAL_AI_REPORT
        ...
        for period, report in text_reports.items():
            if "AI" in period or "REPORT" in period: continue
            html_content += f'<h4>📊 {period} 報酬分布</h4><pre style="background:#2d3436; color:#fff; padding:10px; font-size:12px;">{report}</pre>'
