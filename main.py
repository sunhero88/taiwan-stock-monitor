# main.py
# =========================================================
# Sunhero｜股市智能超盤中控台（Predator V16.3.15-STABLE）
# 針對「網路/雲端環境」的最終穩定版
#
# 關鍵修正 (V16.3.15)：
# 1. [假日修復] Yahoo 抓取改為 period="5d"，解決週六日抓不到週五數據導致的保底誤判。
# 2. [防當機] 維持 TopN=8 與 GC 機制，保護雲端主機記憶體。
# 3. [策略] 鉅亨網 API (優先) -> Yahoo 解析 -> Yahoo 估算 -> 保底 1700億。
# =========================================================
...
# =========================
st.set_page_config(
    page_title="Sunhero｜股市智能超盤中控台（Predator V16.3.15）",
    layout="wide",
)

APP_TITLE = "Sunhero｜股市智能超盤中控台（TopN + 持倉監控 / V16.3.15-STABLE）"
st.title(APP_TITLE)

# =========================
# Global Constants
# =========================
# [CRITICAL] 雲端環境資源有限，強制預設較小的監控數量
DEFAULT_TOPN = 8 
...
def _fetch_twse_robust(trade_date: str) -> Tuple[int, str]:
    ...
    # 策略 2: Yahoo 估算 (修正：抓5天防假日空值)
    try:
        t = yf.Ticker("^TWII")
        # [FIX] 改成 5d，確保週六日能抓到週五數據
...
def _fetch_tpex_robust(trade_date: str) -> Tuple[int, str]:
    ...
    # 策略 3: Yahoo Finance 估算 (修正：抓5天防假日空值)
    try:
        t = yf.Ticker("^TWO")
        # [FIX] 改成 5d，確保週六日能抓到週五數據
        h = t.history(period="5d") 
...
def main():
    ...
    run_btn = st.sidebar.button("啟動中控台 (V16.3.15)")
